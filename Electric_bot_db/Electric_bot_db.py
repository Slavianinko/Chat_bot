...
def bot(text):
    """ Принимаем текст, ищем по колонке Индекс
        Если совпадает однозначно - возвращаем полную информацию
        Если больше одного совпадения - возвращаем список совпадений
    """
    # Поиск по колонке Индекс
    result = df[df['Индекс'].str.contains(text)]
    # Если нет совпадений
    if result.shape[0] == 0:
        return 'Не найдено'
    # Если одно совпадение
        ...
        for i_col in df.columns:
            if pd.notna(result[i_col].values[0]):
                answer += i_col + ': ' + str(result[i_col].values[0]) + '\n'
    else:
    # Если больше одного совпадения
        answer = 'Найдено: \n'
        for item in result['Индекс'].values:
            answer += item + '\n'
    return answer
...
col = [
    ...
    'Извещение об подключении Дата1',
